...
# define model
# weights = {0:1.0, 1:round(len(y_test[y_test == 0])/len(y_test[y_test == 1]), 2)}
# model = SVC(gamma='scale', probability=True, class_weight=weights)
model = SVC(gamma='scale', probability=True, class_weight='balanced', shrinking=True, random_state=0)
# define evaluation procedure
cv = RepeatedStratifiedKFold(n_splits=5, n_repeats=3, random_state=1) #StratifiedKFold(n_splits=3, random_state=1, shuffle=True)
# evaluate model
...
plt.plot(lr_fpr, lr_tpr, marker='.', label='SVM: AUC=%.3f' % (lr_auc))
# axis labels
...
plt.plot(lr_recall, lr_precision, marker='.', label='SVM: f1=%.3f auc=%.3f' % (lr_f1, lr_auc))
# axis labels
plt.xlabel('Recall')
